...
class DataSource(object):
    ...
    def update_stock_data(self,symbols,from_date,to_date):
    ...
    def read_stock_data(self,symbols):
        stock_data = {}
        for symbol in symbols:
            stock_data_file = self._base_dir + '/' + symbol + '.csv'
            if os.path.exists(stock_data_file):
                history_data = pd.read_csv(stock_data_file, index_col='Date',
                                           ...
                                           dtype={'Open': np.float, 'High': np.float, 'Low': np.float,
                                                  'Close': np.float, 'Volume': np.float})
                stock_data[symbol] = history_data
